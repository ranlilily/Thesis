...
def make_melody_symbol_sequences(songs, time_signature, acceptable_durations, save_dir):
  ...
  for i, song in enumerate(songs):      
    ...
    song = transpose_key(song, key)
    song = transpose_octave(song) 

    ## (v) encode melody to symbol sequence then merge them
    encoded_seq = encode_sequence_char(song)
